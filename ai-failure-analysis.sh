...
# Receives it from analyzeTestFailuresWithAI(migrationqe-automation/vars/oadpQeAutomation.groovy)
EXIT_CODE="${1:-0}"
...
# Write a redacted copy before passing to Codex 
redact_file() {
    local src="$1"
    local dst="$2"
    redact_secrets <"$src" >"$dst"
}
...
extract_log_errors() {
    ...
    local redacted_dir="${ARTIFACT_DIR}/.redacted"
    mkdir -p "${redacted_dir}"
    local redacted_log="${redacted_dir}/$(basename "$log_file")"
    
    redact_file "$log_file" "$redacted_log"

    # Use the $redacted_log going forward, never the raw log_file
    if [ "$file_size" -lt "$LARGE_FILE_THRESHOLD" ]; then
        echo "=== Log: $(basename "$log_file") (${file_size} bytes) ===" >>"$output_file"
        head -n 50 "$redacted_log" >>"$output_file"
        echo "..." >>"$output_file"
        tail -n 100 "$redacted_log" >>"$output_file"
        return 0
    fi
...
Do NOT include debug/info level messages unless they are from the same package as an error and occurred within 10 lines before it." 2>>"${AI_LOG_FILE}")
    ...
    if [ $? -eq 0 ] && [ -n "$subagent_output" ]; then
        ...
        echo "$subagent_output" \vert{} head -n 200 >>"$output_file"
    else
        echo "=== Log: $(basename "$log_file") (fallback grep) ===" >>"$output_file"
        grep -i -E '(error|fatal|panic|failed|timeout|exception)' "$redacted_log" 2>/dev/null | tail -n 100 >>"$output_file"
    fi
}
...
" 2>/dev/null && echo "✓ Created failed-specs.json (reduced token input)" || echo "⚠ Could not preprocess report.json, AI will use full file"
    ...
    # Create a redacted copy of the artifacts before passing to Codex
    REDACTED_DIR="${ARTIFACT_DIR}/.ai-redacted"
    echo "Creating isolated & redacted artifact directory at ${REDACTED_DIR}..."
    mkdir -p "${REDACTED_DIR}/logs" "${REDACTED_DIR}/oadp-ref"

    [ -f "${ARTIFACT_DIR}/failed-specs.json" ] && redact_file "${ARTIFACT_DIR}/failed-specs.json" "${REDACTED_DIR}/failed-specs.json"
    [ -f "${ARTIFACT_DIR}/preprocessed-logs.txt" ] && redact_file "${ARTIFACT_DIR}/preprocessed-logs.txt" "${REDACTED_DIR}/preprocessed-logs.txt"
    [ -f "${ARTIFACT_DIR}/junit_report.xml" ] && redact_file "${ARTIFACT_DIR}/junit_report.xml" "${REDACTED_DIR}/junit_report.xml"
    [ -f "${FLAKES_FILE}" ] && redact_file "${FLAKES_FILE}" "${REDACTED_DIR}/oadp-ref/flakes.go"

    if [ -d "${ARTIFACT_DIR}/logs" ]; then
        find "${ARTIFACT_DIR}/logs" -type f | while read -r log_file; do
            rel_path="${log_file#${ARTIFACT_DIR}/logs/}"
            mkdir -p "${REDACTED_DIR}/logs/$(dirname "$rel_path")"
            redact_file "$log_file" "${REDACTED_DIR}/logs/$rel_path"
        done
    fi

    # Build --add-dir flags pointing to REDACTED_DIR instead of ARTIFACT_DIR
    ADD_DIR_FLAGS="--add-dir ${REDACTED_DIR}"
    ...
    ##########################
    cat >"${REDACTED_DIR}/ai-prompt.txt" <<PROMPT_EOF
...
- File: \`${REDACTED_DIR}/failed-specs.json\` (preprocessed, contains ONLY failed specs)
...
2. Do not include/print any sensitive information (API keys, tokens, passwords or credentials) in your output.
...
Read the full analysis instructions in: ${REDACTED_DIR}/ai-prompt.txt

Analyze these artifacts IN THIS ORDER:
1. Failed specs: ${REDACTED_DIR}/failed-specs.json (START HERE — preprocessed, only failed tests) 2. Preprocessed log errors:${REDACTED_DIR}/preprocessed-logs.txt for these failed tests
3. Per-test must-gather: ${REDACTED_DIR}/logs/*/  (find oadp-must-gather-summary.md inside)
4. JUnit report: ${REDACTED_DIR}/junit_report.xml (supplementary) 5. Known flakes:${REDACTED_DIR}/oadp-ref/flakes.go (if exists)
...
IMPORTANT: Do NOT include any API keys, tokens, passwords, or credentials in your output." >> "${AI_LOG_FILE}" 2>&1

    CODEX_EXIT=$?

    # Apply secret redaction to output (Still maintained as last line of defense)
    if [ -f "$TEMP_OUTPUT" ] && [ -s "$TEMP_OUTPUT" ]; then
        redact_secrets <"$TEMP_OUTPUT" >"${ARTIFACT_DIR}/ai-failure-analysis.md"
    fi

    if [ $CODEX_EXIT -eq 0 ]; then
        echo "✓ AI analysis completed successfully "
        echo "✓ Analysis saved to: ${ARTIFACT_DIR}/ai-failure-analysis.md"
    elif [ $CODEX_EXIT -eq 124 ]; then
        echo "✗ AI analysis timed out after 10 minutes"
        echo "Partial analysis may be in ${ARTIFACT_DIR}/ai-failure-analysis.md"
    else
        echo "✗ AI analysis failed (exit code: $CODEX_EXIT)"
        echo "Check ${ARTIFACT_DIR}/ai-failure-analysis.md for error details"
        exit 1
    fi
