...
#   SKIP_AI_ANALYSIS             — Set to "true" to skip (default: false)
...
SKIP_AI=${SKIP_AI_ANALYSIS:-false}
EXIT_CODE=$1
...
" 2>/dev/null && echo "✓ Created failed-specs.json (reduced token input)" || echo "⚠ Could not preprocess report.json, AI will use full file"
    ...
    #############################################################################################################
    # Create analysis prompt (TODO : update to include more context and details for better accuracy for analysis)
    #############################################################################################################
...
PROMPT_EOF
    ...
    echo "Invoking Codex for analysis..."
    ...
    # --add-dir grants Codex read access to artifact directories and test source
    # --sandbox danger-full-access (Work around, Codex CLI sandbox read-only mode not allowing to inspect or correlate the files without fabricating results,
    # switch to danger-full-access — this skips bubblewrap entirely.)
    # --sandbox read-only prevents any file modifications
    # --ephemeral prevents session files from persisting
    # -o writes the final analysis to the temp file
...
2. Preprocessed log errors: ${ARTIFACT_DIR}/preprocessed-logs.txt for thid failed tests
